...
async function main() {
  ...
  console.log('Overall Attempts Summary:', totalAttempts);
  ...
  console.table(diffBreakdown);

  // 4. Concept / Tag Level breakdown
  const [conceptBreakdown] = await conn.query(
    'SELECT s.name as skillName, q.tags, count(a.id) as attempts, sum(case when a.is_correct=1 then 1 else 0 end) as correct, round(sum(case when a.is_correct=1 then 100.0 else 0.0 end)/count(a.id), 1) as accuracy FROM student_question_attempts a JOIN questions q ON q.id = a.question_id JOIN skills s ON s.id = q.skill_id WHERE a.student_id = ? GROUP BY s.name, q.tags ORDER BY attempts DESC',
    [studentId]
  );
  console.log('Top Concepts:');
  console.table(conceptBreakdown.slice(0, 30));

  // 5. Specific CSS questions attempted
  const [cssQ] = await conn.query(
    'SELECT q.title, q.tags, a.is_correct, a.time_spent_seconds, a.created_at FROM student_question_attempts a JOIN questions q ON q.id = a.question_id JOIN skills s ON s.id = q.skill_id WHERE a.student_id = ? AND s.name = "CSS" ORDER BY a.created_at DESC LIMIT 20',
    [studentId]
  );
  console.log('Recent CSS Questions Attempted:');
  console.table(cssQ);

  // 6. Registered profile skills
