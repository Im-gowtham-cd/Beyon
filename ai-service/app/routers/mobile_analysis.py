...
async def analyze_mobile_frame(req: MobileFrameRequest):
    ...
    try:
        ...
        # Person detection via skin regions + head level separation
        ...
        # Candidate heads: only in upper 45% of frame (arms/hands on desk are in y > 0.45 * h)
        head_candidates = []
        min_head_area = 0.04 * (w * h)
        for c in contours:
            area = cv2.contourArea(c)
            if area > min_head_area:
                x, y, cw, ch = cv2.boundingRect(c)
                # Genuine head must have its top edge in upper 45% of frame
                if y < 0.45 * h:
        ...
        if len(head_candidates) == 1:
            ...
            for other in head_candidates[1:]:
                ...
                # Distinct horizontal separation (> 35% frame width) AND both tops in upper frame
        ...
        # Secondary phone screen detection (Must be an illuminated, powered-on rectangular screen)
        # Rejects books, paper, desk pads, laptop bezels, mouse pads
        secondary_device = False
        blurred = cv2.GaussianBlur(gray, (5, 5), 0)
        # Look for high luminance regions (active screen emitting light)
        ...
        for c in screen_contours:
            ...
            if len(approx) == 4:
                ...
                area = cw * ch
                # Phone sized: between 3% and 12% of frame
                if 0.03 * (w * h) < area < 0.12 * (w * h):
                    aspect = float(max(cw, ch)) / max(float(min(cw, ch)), 1.0)
                    if 1.6 <= aspect <= 2.3:
                        # Compute mean brightness inside quad to confirm illuminated screen
